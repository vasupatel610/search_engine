...
def get_smart_suggestions(query: str, limit: int = 10):
    """Enhanced suggestions with Amazon-like completed queries, focusing on categories and gender relevance, excluding brands"""
    if not query:
        return []
    ...
    # For multi-character queries, use fuzzy matching for categories only
    # 1. Fuzzy match categories
    ...
    # 2. Add pure category suggestions if match
                ...
                for gender in ['Men', 'Women']:
                    if gender in expected_genders or 'Unisex' in expected_genders:
                        ...
                        if gender_suggestion not in seen:
                            ...
                            seen.add(gender_suggestion)
    # 3. Add brand-category combinations only if brand is part of the query
    if any(word in query_clean for word in popular_brands):
        for brand in popular_brands:
            if brand.lower() in query_clean:
...
async def suggestions_endpoint(q: str = Query(..., min_length=1), limit: int = 10):
    """API endpoint for enhanced search suggestions with gender relevance, excluding brands"""
...
if initialize_app():
    ...
    <div style="position: relative; display: inline-block; width: 100%;">
        <input type="text" id="searchInput" placeholder="Try: 'shoes', 'nike shoes for men', 's' for shirts..." 
